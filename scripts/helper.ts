...
async function deployContract<T>(name: string, args: any[], label?: any, options?: any) {
  ...
  writeDeployedContract(name, contract.address, args);
...
function writeDeployedContract(name: string, contract: string, args: any[]) {
  ...
  if (!fs.existsSync(filePath)) {
    fs.writeFileSync(
      filePath,
      JSON.stringify({ [contract]: { contract: contract, args: args } }, null, 2)
    );
  } else {
...
function readFromFile(contract: string): string {
  const network = process.env.HARDHAT_NETWORK || 'hardhat';
  const filePath = `./deployed/${network}-addresses.json`;
  if (!fs.existsSync(filePath)) {
    return '';
